defmodule Mix.Tasks.Package.Android.Runtime do
  ...
  def run(nifs) do
    ...
    Enum.each(nifs, fn nif -> MobileRuntimes.get_nif(nif) end)
  ...
  def build(archid, extra_nifs) do
    ...
    if File.exists?(runtime_target(arch)) do
      ...
      if !File.exists?(otp_target(arch)) do
        MobileRuntimes.ensure_otp()
        cmd(~w(git clone _build/otp #{otp_target(arch)}))
        # Patches are already applied and committed in _build/otp via apply_otp_patches()
      end
      ...
      extra_nifs =
        Enum.map(extra_nifs, fn nif ->
          if static_lib_path(arch, MobileRuntimes.get_nif(nif)) == nil do
            Nif.build(archid, nif)
          end

          static_lib_path(arch, MobileRuntimes.get_nif(nif))
          |> Path.absname()
        end)
